...
const debug = require('debug')('calculator:main');

// ## 计算器

// 转换为固定精度数值
const parseDecimal = (v, decimal) => parseInt(parseFloat(v) / decimal, 10) * decimal;
// 转换为固定精度百分比
const parsePercent = (v, decimal) => parseInt(parseFloat(v) / decimal * 100, 10) * decimal;
...
const calcRatio = (openPrice, closePrice, fee, leverage) => {
  ...
  // debug('做多盈利率 %s%, 做空亏损率 %s%', longRatio * 100, shortRatio * 100);
...
const calcPrice = (openPrice, ratio, fee, leverage) => {
  ...
  // debug('做多止盈/做空止损价格 %s, 做空止盈/做多止损价格 %s', upperPrice, lowerPrice);
...
const triggerShort = (openPrice, winRatio, loseRatio, fee, leverage) => {
  ...
  // debug('Close Short: 止盈价格 %s, 止损价格 %s', lowerPrice, upperPrice);
