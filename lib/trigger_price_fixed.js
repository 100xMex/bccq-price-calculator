...
class FixedTriggerPrice extends EventEmitter {
  ...
  addCont(cont, price) {
    ...
    return this.onPriceChange(currPrice);
  }
  ...
  onPriceChange(price) {
    ...
    debug('价格 %s 未触发止盈 %s=>%s 止损 %s=>%s', this.currPrice, this.triggerWinPrice, this.closeWinPrice, this.triggerLosePrice, this.closeLosePrice);
